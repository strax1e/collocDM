...
INT_PTR CALLBACK PolynomProc( HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam )
{
	...
	case WM_INITDIALOG:
	{
		...
		CreateWindow(TEXT("BUTTON"), TEXT("Производная A"), WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_PUSHBUTTON, 228, cury += 40, 180, 20, hWnd, (HMENU)0x213, hInst, NULL);
		CreateWindow(TEXT("BUTTON"), TEXT("Производная B"), WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_PUSHBUTTON, 228, cury += 40, 180, 20, hWnd, (HMENU)0x214, hInst, NULL);
		CreateWindow(TEXT("BUTTON"), TEXT("Кратные корни A в простые"), WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_PUSHBUTTON, 188, cury += 40, 260, 20, hWnd, (HMENU)0x215, hInst, NULL);
